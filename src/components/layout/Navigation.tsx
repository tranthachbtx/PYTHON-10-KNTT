...
export function DesktopSidebar({
  ...
  const filteredLessons = lessons.filter(
    (l) => l.subject === currentSubject && l.grade === currentGrade,
  );
            ...
            <div className="mb-4">
              {sidebarState === "full" && (
                <div className="px-4 mb-6 flex items-center justify-between">
                  <p className="text-[11px] font-black text-white/20 uppercase tracking-[0.3em] leading-none">
                    Danh sách bài học
                  </p>
                  <span className="text-[10px] font-black py-1 px-2 bg-white/10 text-white/60 rounded-lg">
                ...
                {filteredLessons.map((lesson) => (
                  <Link
                    ...
                    className={cn(
                      ...
                      pathname ===
                        `/lessons/${lesson.subject}/${lesson.grade}/${lesson.slug}`
    ...
    >
      <div
        className={cn(
          ...
          active
            ? "bg-slate-900 text-white shadow-lg rotate-12"
...
export function Navigation({ lessons }: NavigationProps) {
  ...
  const isLessonPage =
    pathname.startsWith("/lessons/") && pathname.split("/").length >= 4;
    ...
    <nav>
      ...
      {isLessonPage && (
        <>
          <button
            onClick={() => setMobileMenuOpen(true)}
            className="md:hidden fixed top-4 right-4 z-50 w-12 h-12 glass shadow-lg rounded-2xl flex items-center justify-center text-slate-900 border border-white/40 active:scale-90 transition-all"
          >
            <Menu size={24} />
          </button>
          <MobileDrawer
